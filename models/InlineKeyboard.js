...
class InlineKeyboard {
  static async departments() {
    ...
    try {
      ...
      rows.forEach((row) => {
        const departmentCapitalized = row.department_title.charAt(0).toUpperCase()
          + row.department_title.slice(1);
        departments.push(
          [
            {
  ...
  static async teachers(departmentId) {
    ...
    try {
      ...
      const [rows] = await connection.query(
        'SELECT id, last_name, first_name, patronymic FROM Teachers WHERE department=?', departmentId,
      );
      connection.end();
